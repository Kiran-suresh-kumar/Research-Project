...
def gIHT(X, y, groups, lasso_lam, max_iter, sub_iter, tol, print_flag=False):
    """ Solves group sparse linear regression using the gIHT algorithm

    Args:
        X: The data matrix, of shape (n x d)
        y: The target vector, of shape (n x 1)
        groups: A list of group indices, where groups[i] is the set of indices of
                features in group i.
        lasso_lam: The regularization hyperparameter
        max_iter: The maximum number of iterations
        sub_iter: The number of iterations per debiasing step
        tol: The tolerance for convergence
        print_flag: Whether to print convergence information
    ...
    n, d = X.shape
    coeff_old = np.zeros((len(groups), d))
    coeff_new = np.zeros((len(groups), d))
   

    # Initialize the coefficients
    for i in range(len(groups)):
        init = np.linalg.lstsq(X[:, groups[i]], y, rcond=None)[0].reshape(1, d)
        coeff_old[i, :] = init + 0.01 * np.random.normal(0, 1, d)
    total_frac_iter = 0
    for iteration in range(0, max_iter):
        # Gradient descent step associated with the data-fitting objective
        temp = np.zeros((len(groups), d))
        for i in range(len(groups)):
            temp[i] = coeff_old[i] + (2.0 / np.linalg.norm(X[:, groups[i]].T.dot(X[:, groups[i]]), 2)) * (
                init - X[:, groups[i]].T.dot(y)
            ).reshape(1, d)
        ...
        biginds = []
        for i in range(len(groups)):
            smallinds_i = np.where(abs(temp[i]) <= lasso_lam)[0]
            biginds_i = [j for j in range(d) if j not in smallinds_i]
            smallinds.append(smallinds_i)
            biginds.append(biginds_i)
        ...
        for i in range(len(groups)):
            ...
            for _ in range(sub_iter):
                coeff_new[i], iter_frac_update = debiasing(X[:, groups[i]], y, temp[i], coeff_new[i], smallinds[i], biginds[i], 1, lasso_lam)
                frac_iter += iter_frac_update# Update frac_iter for the current group
            total_frac_iter += frac_iter    
        # Check for convergence
        if (np.linalg.norm(coeff_new - coeff_old, 'fro') < tol):
            if print_flag:
                # Use total_frac_iter for all groups divided by (iteration + 1)
                print("Converged at iteration", iteration, "frac_iter", total_frac_iter / (iteration + 1))
            return coeff_new
